...
*/
if(isset($_POST["submit"], $_POST["d1"],$_POST["d2"]))
{
    // echo 'working';
    $ta=0;
    $tk=0;
    ...
    $tpay=0;
    // $name=$_POST["name"];
    $start=date("Y-m-d", strtotime($_POST["d1"]));
    $end=date("Y-m-d", strtotime($_POST["d2"]));
    if(date("d", strtotime($end))=="31"){ $end=date("Y-m-30");}
    $i=$i+1;
    //$query="SELECT DISTINCT `register`.`cid` AS `cid` FROM `register` WHERE ";
    $query="SELECT DISTINCT `register`.`cid` AS `cid`, `register`.`full` AS `fullname`,`register`.`address` as `place`, `register`.`bank` AS `bank`,`register`.`account` AS `acc_no`,`register`.`ifsc` AS `isfc`, `register`.`pan` AS `pan_card_num` FROM `register` ORDER BY `register`.`cid`";
     $confirm_query=mysqli_query($conn,$query) or die(mysqli_error());
    while($row=mysqli_fetch_array($confirm_query))
    {
