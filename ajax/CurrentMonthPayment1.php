...
if(isset($_POST['currentMonthPayment']))
{
    ...
    if ($result->num_rows > 0)
    {
        while ($row = $result->fetch_assoc()) 
        {
            ...
            $qu="SELECT `referal`.`refasign`,`referal`.`id`,`invest`.`invest`,`invest`.`regdate` FROM `referal`,`invest` WHERE `referal`.`id`=`invest`.`id` AND `invest`.`regdate`<'$end' AND `referal`.`refcid`='$cid'";
            // $qu="SELECT ";
            ...
            if ($result2->num_rows > 0) 
            {
                while ($out = $result2->fetch_assoc()) 
                {
                    ...
                    $invest_date1=$out['regdate'];
                    $w_value=widraw($conn,$id,$end);
                    $days1=date_calculation($invest_date1,$start,$end,$srch_date,$currentMonthPayment);
                    $amt=$out['invest']+$w_value;
                    $perc=$out['refasign'];
                    $day_amt=(($amt*$perc/100)/30);
...
if(isset($_POST['currentMonthPayment']))
{
    ...
    if($currentMonthPayment=='currentMonthPayment')
    {
        if($k1==0)
        {
            $limit=$_POST['limit'];
            $std=$_POST['start'];
            $sql = "SELECT * FROM `register` WHERE `cid`<=200 ORDER BY `cid` ASC LIMIT $std, $limit;";
        }else if($k1==1)
        {
            $cid=$_POST['cid'];
            $sql = "SELECT * FROM `register` WHERE `cid`='$cid'";
