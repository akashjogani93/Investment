                ...
                <div class="box">
          	 	...
          	 	<div class="table-responsive">
                    ...
                    <thead>
                        ...
                        <input type="text" value="<?php echo $id; ?>" name="id" class="form-control form-control-sm" required="" style="display:none;">
...
if(isset($_POST['New_pass']))
{
	...
	{
		$query="SELECT * FROM `login` WHERE `cid`='$id' AND `password`='$password' AND `user`='Member';";
		$confirm=mysqli_query($conn,$query) or die(mysqli_error());
		if(mysqli_num_rows($confirm) > 0)
		{
			$query="UPDATE `login` SET `password`='$confirm_pass' WHERE `user`='Member' AND `cid`='$id';";
			$confirm=mysqli_query($conn,$query) or die(mysqli_error());
			if($confirm)
			{
		...
		}else
        {
		    ...
		    echo "<script>location='member_change_pass.php';</script>";
        }
	}else{
		echo "<script>alert('Password Wrong, Try Again');</script>";
		echo "<script>location='member_change_pass.php';</script>";
